/**
 * Seed the SRC-CQR Worldview Matrix v1.0 + FrameworkConfig v1.
 *
 * Source: SRC_Content_Classification_Rating_Framework.docx §2 (Matrix) + §5 (weights/thresholds/flags).
 * Idempotent: upserts domains by code, replaces that domain's entries, upserts FrameworkConfig v1.
 *
...
async function main() {
  ...
  for (const d of MATRIX) {
    ...
    // Replace this domain's entries (clean re-seed)
    ...
    console.log(`  ${d.code}: ${d.entries.length} entries`);
  }
  ...
  console.log(`\nSeeded ${MATRIX.length} domains, ${entryCount} entries, FrameworkConfig v1 (published).`);
}
