...
// POST /api/reports2 — Ingest a new report from vnOrchestrator
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (ticketReason) {
      ...
      return NextResponse.json(
        {
          error: `This submission looks like an internal workflow ticket (${ticketReason}), not a publishable report. Only finished editorial content should be posted to /api/reports2.`,
...
// GET /api/reports2 — Fetch reports (public: published only; with auth: all)
// Query params:
